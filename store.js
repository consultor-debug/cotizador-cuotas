...
(function () {
  ...
  function seed(APP) {
    ...
    // ---- bitácora (logs de auditoría) derivada de la actividad demo ----
    const mkId = (t) => "log-" + t.toString(36) + Math.floor(rnd() * 1e6).toString(36);
    const logs = [];
    const COT_LBL = { enviada: "Enviada", aceptada: "Aceptada", rechazada: "Rechazada", vencida: "Vencida" };
    cotizaciones.forEach(c => {
      logs.push({ id: mkId(c.ts), ts: c.ts, actorId: c.asesorId, actorNombre: c.asesorNombre,
        cat: "cotizacion", accion: "enviar", detalle: "Cotización " + c.id + " enviada · lote " + c.loteId + " · " + c.clienteNombre, ref: c.id });
      if (c.estado === "aceptada") {
        logs.push({ id: mkId(c.ts + 1), ts: c.ts + 3600000, actorId: c.asesorId, actorNombre: c.asesorNombre,
          cat: "venta", accion: "convertir", detalle: "Lote " + c.loteId + " vendido a " + c.clienteNombre, ref: c.loteId });
      } else if (c.estado === "rechazada" || c.estado === "vencida") {
        logs.push({ id: mkId(c.ts + 2), ts: c.ts + 7200000, actorId: c.asesorId, actorNombre: c.asesorNombre,
          cat: "cotizacion", accion: "estado", detalle: "Cotización " + c.id + " → " + COT_LBL[c.estado], ref: c.id });
      }
    });
    reservas.forEach(r => {
      (r.historial || []).forEach((h, i) => {
        const cat = h.accion === "convertida" ? "venta" : "reserva";
        const det = h.accion === "creada" ? "Lote " + r.loteId + " separado para " + r.clienteNombre + " · " + (r.dias || 3) + " días"
          : h.accion === "convertida" ? "Lote " + r.loteId + " vendido a " + r.clienteNombre + " (desde reserva)"
          : h.accion === "liberada" ? "Reserva del lote " + r.loteId + " liberada"
          : h.accion === "plazo" ? "Lote " + r.loteId + " · plazo " + (h.detalle || "")
          : "Lote " + r.loteId + " vencido y liberado automáticamente";
        logs.push({ id: mkId(h.ts + i), ts: h.ts, actorId: h.por === "Sistema" ? null : r.asesorId, actorNombre: h.por || r.asesorNombre, cat, accion: h.accion, detalle: det, ref: r.loteId });
      });
    });
    // un par de ajustes de inventario por gerencia
    const ger = comerciales.find(a => /gerente|subgerente|coordinador/i.test(a.rol)) || comerciales[0];
    if (ger && disponibles.length) {
      logs.push({ id: mkId(now - 6 * DAY), ts: now - 6 * DAY, actorId: ger.id, actorNombre: ger.nombre,
        cat: "lote", accion: "editar", detalle: "Lote " + disponibles[0].id + " editado · precio actualizado", ref: disponibles[0].id });
    }
    logs.sort((a, b) => b.ts - a.ts);

    return { clientes, cotizaciones, reservas, logs };
  }
