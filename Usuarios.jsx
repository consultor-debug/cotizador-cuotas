...
function Usuarios({ asesores, setAsesores, currentId, toast, onLog }) {
  ...
  async function guardar(u) {
    ...
    if (onLog) onLog({ cat: "usuario", accion: "editar", detalle: "Accesos de " + u.nombre + " actualizados (" + u.rol + ")", ref: u.id });
  ...
  async function eliminar(u) {
    ...
    if (onLog) onLog({ cat: "usuario", accion: "eliminar", detalle: "Cuenta de " + u.nombre + " eliminada", ref: u.id });
    setDel(null);
  }
  async function toggleActivo(a) {
    ...
    if (onLog) onLog({ cat: "usuario", accion: bloquear ? "bloquear" : "habilitar", detalle: (bloquear ? "Acceso bloqueado a " : "Acceso habilitado a ") + a.nombre, ref: a.id });
  }
  ...
  return (
    <Modal onClose={onClose} title={isNew ? "Crear usuario" : "Editar accesos"} width={500}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        {/* Alcance del trabajo comercial: ver vs editar */}
        <div style={{ fontSize: 11.5, fontWeight: 800, letterSpacing: ".05em", textTransform: "uppercase", color: "var(--faint)", margin: "8px 2px 0" }}>Alcance de reservas y cotizaciones</div>
        <div style={{ fontSize: 12, color: "var(--muted)", margin: "0 2px 2px" }}>Sin estos, el asesor solo ve y gestiona <b>lo que hizo con su propia cuenta</b>.</div>
        {PERMS.SCOPE_KEYS.map(k => {
          const on = !!acc[k.k];
          return (
        ...
        <div style={{ fontSize: 11.5, fontWeight: 800, letterSpacing: ".05em", textTransform: "uppercase", color: "var(--faint)", margin: "10px 2px 0" }}>Accesos a módulos</div>
