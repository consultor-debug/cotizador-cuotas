...
function SchematicBg() {
  return (
    <div style={{ position: "absolute", inset: 0, borderRadius: 10, background:
      "repeating-linear-gradient(0deg,#efe7d7,#efe7d7 2px,transparent 2px,transparent 26px),repeating-linear-gradient(90deg,#efe7d7,#efe7d7 2px,transparent 2px,transparent 26px),#f3ede1" }}>
      <div style={{ position: "absolute", left: 600, top: 0, width: 40, height: 596, background: "#e6dcc6" }}></div>
      <div style={{ position: "absolute", top: -4, left: 620, fontSize: 9, fontWeight: 800, letterSpacing: ".1em", color: "#a9966f", transform: "translateX(-50%)" }}>▼ INGRESO</div>
      {PLAN.AMENITIES.map(a => <Amenity key={a.id} a={a} />)}
      {PLAN.LAYOUT.map(mz => (
        <div key={mz.id} style={{ position: "absolute", left: mz.x - 1, top: mz.y - 15, fontSize: 11, fontWeight: 800, color: "#8a7a58", fontFamily: "var(--font-display)" }}>{mz.id}</div>
      ))}
    </div>
  );
}
function Amenity({ a }) {
  const s = {
    park: { bg: "radial-gradient(circle at 50% 40%,#bcd9a0,#9cc47e)", label: "#4a6b30", icon: "🌳" },
    pool: { bg: "linear-gradient(135deg,#cfeafe,#a9d6f7)", label: "#2c6a93", icon: "🏊" },
    soccer: { bg: "linear-gradient(180deg,#a9cf86,#8fc06a)", label: "#3f6128", icon: "⚽" },
  }[a.kind];
  return (
    <div style={{ position: "absolute", left: a.x, top: a.y, width: a.w, height: a.h, background: s.bg,
      border: "2px solid rgba(255,255,255,.6)", borderRadius: 8, display: "flex", flexDirection: "column",
      alignItems: "center", justifyContent: "center", boxShadow: "inset 0 0 0 1px rgba(0,0,0,.05)" }}>
      <div style={{ fontSize: a.kind === "park" ? 30 : 20 }}>{s.icon}</div>
      <div style={{ fontSize: 10.5, fontWeight: 800, color: s.label, letterSpacing: ".04em", marginTop: 4, textTransform: "uppercase" }}>{a.label}</div>
    </div>
  );
}
