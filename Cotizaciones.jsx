...
function Cotizaciones({ cotizaciones, setCotizaciones, asesor, perms, moneda, toast }) {
  const [filtro, setFiltro] = useState("todas");
  const [q, setQ] = useState("");
  const [menu, setMenu] = useState(null);

  const mias = perms.admin ? cotizaciones : cotizaciones.filter(c => c.asesorId === asesor.id);
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 16, flexWrap: "wrap" }}>
          <div>
            ...
            <div style={{ color: "var(--muted)", marginTop: 6, fontSize: 14.5 }}>
              {perms.admin ? "Historial de todo el equipo comercial." : "Tu historial de cotizaciones enviadas."} {mias.length} en total.
            </div>
          </div>
        </div>
