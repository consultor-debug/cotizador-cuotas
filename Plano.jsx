...
function Plano({ lotes, setLotes, polys, setPolys, planoImg, setPlanoImg, planoMode, setPlanoMode, planoOpacity, setPlanoOpacity, cond, asesor, moneda, perms, brand, clientes, onEnviar, onReservar, onCerrarReserva, toast }) {
  ...
  function onAction(k) {
    ...
    if (k === "vendido") { setEstado(sel.id, "vendido"); onCerrarReserva && onCerrarReserva(sel.id, "convertida"); toast("Lote " + sel.id + " marcado como vendido por " + asesor.nombre, "ok"); }
    if (k === "liberar") { setEstado(sel.id, "disponible"); onCerrarReserva && onCerrarReserva(sel.id, "liberada"); toast("Lote " + sel.id + " liberado", "ok"); }
    if (k === "no_disponible") { setEstado(sel.id, "no_disponible"); onCerrarReserva && onCerrarReserva(sel.id, "liberada"); toast("Lote " + sel.id + " marcado como no disponible para venta", "warn"); }
    if (k === "transp") {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0, overflowY: "auto" }}>
      {/* Header */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 16, padding: "22px 30px 16px", flexShrink: 0, flexWrap: "wrap" }}>
        <div>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {perms.editarPlano && (
            <div className="segtabs" style={{ padding: 3 }}>
              ...
              <button className={showReal ? "on" : ""} style={seg} onClick={() => { if (hasPlano) setPlanoMode("real"); else fileRef.current.click(); }}><Icon name="pin" size={15} /> Plano real</button>
            </div>
          )}
      ...
      <div style={{ padding: "0 30px 16px", flexShrink: 0 }}>
        <div className="card" style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 16px", flexWrap: "wrap" }}>
      ...
      <div className="plano-split" style={{ flex: 1, display: "flex", gap: 18, padding: "0 30px 24px", minHeight: 0, minBlockSize: 520 }}>
        {/* Lienzo */}
        <div className="card plano-lienzo" style={{ flex: 1, position: "relative", overflow: "hidden", background: planoView ? "#fff" : "#f3ede1" }}>
